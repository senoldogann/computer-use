...
1. **Configurable Autonomy Levels**:
   ...
   - **Level 4 (Sovereign / Explicit Session Delegation)**: The operator deliberately delegates destructive permission for the current bounded session. This level is reachable **only** through the dedicated `--sovereign` switch; it is intentionally absent from numeric `--level` selection and must never be inferred from `--yes`, an unattended session, a model request, or a stored preference. Sovereign may turn a policy-level destructive `CONFIRM` into `ALLOW`, but it is **not** a global safety bypass: a hard deadline/token/cost budget is mandatory, and the kill-switch, coordinate/display bounds, focus/staleness gates, driver trust checks, verification witnesses, completion audit, tracing, stuck-loop guard, recovery ceilings, and model/action schema validation remain active exactly as in lower levels. `--sovereign` is mutually exclusive with `--yes` and with Levels 0–2. Level 3 semantics do not change merely because Level 4 exists.
   - **Delegating in advance (`security/grants.py`)**: answering the same question every night is not autonomy either. A **capability grant** is bounded authority written ahead of time — a verb family (delete / send / pay / install / overwrite / admin / shell, matched across languages so an English grant covers a Turkish button), an application, a glob over control titles, a use count that is really decremented, and an expiry. What a grant may do is deliberately tiny: turn one `CONFIRM` into one `ALLOW` for a `DESTRUCTIVE` action that matches it. Grant lookup therefore happens only after the base policy verdict is `CONFIRM` **and** the classified risk is `DESTRUCTIVE`; routine confirmations never consume grants, and a grant can never lift `DENY`/`BLOCK` or apply at Level 0 or 1. Sovereign is a separate explicit session-wide delegation and does not consume or fabricate grants. Grants are minted with `--grant` (or `--approve <id> --always`), listed with `--grants`, and removed with `--revoke`.
...
3. **Emergency Kill-Switch**: The user must always be able to reclaim physical control instantly. **Wired channels:** **Command+Shift+Escape** (global event-tap hotkey), **rapid cursor shake** (grab the mouse and shake it), and **SIGINT/SIGTERM** (Ctrl-C). The first two live on the same CGEventTap in the driver (`driver/src/hotkey.rs`) and are OR-ed into the `hotkey_state` RPC the orchestrator polls before every step; when tripped, all queued actions are dropped and the run ends.

   The shake was previously detected orchestrator-side and therefore did nothing: every production entry point built the switch with `monitor=None`, and even wired it could not have worked, because the loop polls once per OODA step — seconds apart — while the gesture lasts under a second. It now runs in the tap, which already sees input at HID rate. Two properties make it safe to have on: the rule is *bounded* (≥6 direction reversals inside 0.5 s, total travel ≥40 px, bounding box ≤200 px — the same thresholds as `security/killswitch.py`, so the two detectors cannot disagree), and events this process posted itself are ignored by source pid, so the agent's own Bezier motion can never trip its own kill switch. Both properties are covered by tests, and the discrimination is verified live: 24 self-posted oscillating moves leave it untripped, a foreign process shaking the cursor trips it.

   The driver announces its startup state — `kill_listener_armed` in `health`, kept separate from `trusted` because a missing Accessibility grant and a dead event tap are different failures with different fixes.
...
- **Screenshots/OCR/visual diff = verification**, not generation: candidate coordinates from the AX tree are confirmed by a regional visual diff before acting (`--verify`). Pixel-first OCR grounding is the fallback for apps with no AX tree (games, some Electron apps): `driver/src/vision.rs` reads the screen with Vision.framework and the orchestrator renders those lines into the same summary shape an AX element produces, so they become marks through the existing parser. It engages only when the AX tree came back essentially empty — AX stays primary, and OCR on every turn would cost a text pass per step and duplicate every label the tree already named. If the system-wide AX probe fails, the driver falls back to `CGWindowListCopyWindowInfo` (no consent required) to name the frontmost app, then grounds through that app's per-app AX tree.
...
- **Level 3 remains the default ceiling of ordinary numeric autonomy.** Destructive actions at Full still require an approval record or a matching scoped capability grant.
- **Level 4 is entered only by `--sovereign`.** The dedicated flag is an operator gesture with different semantics from `--yes`; it cannot be selected through `--level`, inferred from unattended operation, or stored as an ambient preference.
- **Sovereign changes one policy decision only:** a destructive action that would otherwise require confirmation may be allowed for that session. It does not disable the runtime safety floor: hard budgets are mandatory and kill-switch, verification, completion audit, trace, focus/staleness checks, coordinate bounds, driver trust, schema validation, stuck-loop detection and recovery ceilings remain in force.
- **Capability grants remain narrower than Sovereign and are consulted narrowly.** They are considered only for `CONFIRM + DESTRUCTIVE`; they neither consume on routine confirmations nor override `DENY`/`BLOCK`. Sovereign does not consume grants because its explicit session delegation already supplies the authority being asked about.
- **Rationale:** trust, scoped standing delegation and explicit session-wide delegation are three different operator decisions. Collapsing them into one boolean makes destructive authority easy to acquire accidentally and impossible to audit correctly.
...
5. [ ] **Safety & Permission Enforced**: Autonomy levels and dangerous action guards are never bypassed; Level 3 keeps destructive approval/grant semantics, while Sovereign is accepted only as the explicit bounded Level 4 delegation defined by Law 5 and ADR-4.
