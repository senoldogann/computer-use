...
3. **Emergency Kill-Switch**: The user must always be able to reclaim physical control instantly. **Wired channels:** **Command+Shift+Escape** (global event-tap hotkey — the driver reports `hotkey_state` and aborts the current action) and **SIGINT/SIGTERM** (Ctrl-C). When tripped, all queued actions are dropped and the run ends. **Not wired: rapid cursor shake.** The detector and its sampling monitor exist and are tested, but every production entry point constructs the switch with `monitor=None`, so grabbing the mouse and shaking it does *nothing* today — the honest thing to say is that the hotkey and Ctrl-C are the escape hatches. Detecting a shake needs the cursor sampled continuously at tens of hertz; the orchestrator polls the switch once per OODA step, seconds apart, which cannot see a burst that lasts under a second. It belongs in the driver's event tap, beside the hotkey tap that already runs there, and that is what wiring it means.
