...
class GoalProposal:
    ...
    Provenance is mandatory: a proposal that cannot say where it came from
    cannot be executed. ``utility_score`` is derived, never stored, so the
    score a proposal was ranked by can never drift from the inputs behind it.
    """
    ...
    def utility_score(self) -> float:
        """Rank score derived from source, confidence and cost (pure)."""
        return proposal_score(self.source_type, self.confidence, self.expected_cost)
...
def make_proposal(
    *,
    ...
    """Build one proposal with mandatory explicit provenance."""
    return GoalProposal(
        goal=goal,
        app=app,
