...
cargo build --manifest-path "$ROOT_DIR/driver/Cargo.toml" --bin actuation-driver --bin actuation-menu

mkdir -p "$MACOS_DIR" "$APP_DIR/Contents/Resources" "$USER_APPS"

echo "==> Packaging as ComputerUse..."
cp "$ROOT_DIR/driver/target/debug/actuation-menu" "$MACOS_DIR/ComputerUse"
cp "$ROOT_DIR/driver/target/debug/actuation-menu" "$MACOS_DIR/actuation-menu"
cp "$ROOT_DIR/driver/target/debug/actuation-driver" "$MACOS_DIR/actuation-driver"
